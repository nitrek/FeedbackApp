...
public class WebVerticle extends AbstractVerticle {
  ...
  Future<Router> configureRouter() {
      ...
      router.get("/api/v1/user").handler(this::helloHandler);
      router.get("/api/v1/user/:name").handler(this::helloByNameHandler);
  ...
  void helloHandler(RoutingContext ctx) {
      vertx.eventBus().request(UserVerticle.GET_USER_DETAILS_ADDR, "", reply -> {
  ...
  void helloByNameHandler(RoutingContext ctx) {
      ...
      vertx.eventBus().request(DatabaseVerticle.GET_USER_DETAILS_FROM_DB_ADDR, name, reply -> {
          ctx.request().response().end((String)reply.result().body());
      });
  }
}
